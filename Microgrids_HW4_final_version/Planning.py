...
#
# In run(), choose between the daily and weekly analysis by uncommenting the desired call.
# The day/week shown can be changed inside plot_res_day() and plot_res_week() in utils.py.
...
def constraint_rule_Pbalance(model, i): 
    return (model.P_pv[i] + model.P_gen[i] + model.P_imp[i] + model.P_discharge_bss[i] + model.P_discharge_ev[i] 
            == model.P_exp[i] + model.P_charge_bss[i] + model.P_charge_ev[i] + model.P_hp_hot[i] + model.P_hp_cold[i] + model.P_load[i])
